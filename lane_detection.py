...
while (cap.isOpened()):
    ...
    lines = Image.fromarray(line_image)
    ...
    canny = ImageTk.PhotoImage(canny)
    ...
    if panelA is None or panelB is None or panelC is None or panelD is None:
        ...
        panelA = Label(image=cropped)
        panelA.image = cropped
        panelA.pack(side="top", padx=10, pady=10)
        # while the second panel will store the edge map
        panelB = Label(image=canny)
        panelB.image = canny
        panelB.pack(side="bottom", padx=10, pady=10)

        panelC = Label(image=lines)
        panelC.image = lines
        panelC.pack(side="left", padx=10, pady=10)

        panelD = Label(image=result)
        panelD.image = result
        panelD.pack(side="right", padx=10, pady=10)
    # otherwise, update the image panels
    else:
        # update the pannels
        panelA.configure(image=cropped)
        panelB.configure(image=canny)
        panelC.configure(image=lines)
        panelD.configure(image=result)
        panelA.image = cropped
        panelB.image = canny
        panelC.image = lines
        panelD.image = result
